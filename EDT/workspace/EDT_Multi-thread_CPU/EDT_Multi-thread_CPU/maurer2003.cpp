...
DWORD WINAPI edt_1d_vertical_kernel(LPVOID lpParam)
{
...
int edt_1d_vertical(DistMap *dm)
{
	...
	for (c=0; c < cols; c++) {
		...
		tmp[num].dm = dm;
		tmp[num].c = c;
		handle[num]=CreateThread(NULL,0,edt_1d_vertical_kernel,&tmp[num],0,NULL);
		num++;
...
DWORD WINAPI edt_maurer_2D_from_1D_kernel(LPVOID lpParam)
{
	Phrase2 *tmp = (Phrase2 *)lpParam;
	DistMap *dm = tmp->dm;
	Pnt *dm_row = tmp->im_row;
	Pnt *g = tmp->g;
	int i1 = tmp->h;
	maurer_voronoi_edt_2D(dm, dm_row,  /* internal: */ g,i1);
	return 0;
}
...
int edt_maurer_2D_from_1D(DistMap *dm)
{
	...
	// OBS: g and h are internal to maurer_voronoi_edt_2d and are
	// pre-allocated here for efficiency.
	g = (Pnt*)malloc(sizeof(Pnt)*ncols*THREADNUM);
	/*animal_malloc_array(int, ncols);
	  if (!g) {
	  animal_err_flush_trace();                                
	  animal_err_register(fname, ANIMAL_ERROR_MALLOC_FAILED,"");  
	...
	for (i1=0; i1 < nrows; ++i1, dm_row += ncols ) {
		tmp[num].dm = dm;
		tmp[num].g = g + num*ncols;
		tmp[num].h = i1;
		tmp[num].im_row = dm_row;

		hdl[num]=CreateThread(NULL,0,edt_maurer_2D_from_1D_kernel,&tmp[num],0,NULL);
		num++;
		if (num==THREADNUM)
		{
			num=0;
...
int remove_edt(Pnt *mlst2,Pnt *mlst1,Pnt *now,int h);
