...
void maPhase1(int m1) 
{
	...
	maKernelPropagateInterband<<< grid, block >>>(maTextures[1], maTexSize, maTexSize / m1); 
	cudaUnbindTexture(maTexColor); 
}
...
{	
	...
	*/
		//int step2 = step/2;
		int taskNum = (maTexSize/step)*(step - 1 );
		//taskNum++;
		...
		cudaBindTexture(0,maTexIcp,maTextures[2]);
		maKernelColor15<<<grid,block>>>(maTextures[3],pppl,maTexSize,step);
		cudaUnbindTexture(maTexIcp);
		cudaUnbindTexture(maTexIcpl);
		...
		block = dim3(BLOCKSIZE);
		grid = dim3((taskNum/BLOCKSIZE)   );
		cudaBindTexture(0,maTexIcpl,pppl);
		cudaBindTexture(0,maTexIcp,maTextures[3]);
		cudaBindTexture(0,maTexP1,maTextures[1]);
		maKernelColor175<<<grid,block>>>(maTextures[4],pppl,maTexSize,step);
		cudaUnbindTexture(maTexIcpl);
		cudaUnbindTexture(maTexIcp);
		cudaUnbindTexture(maTexP1);
		...
		maKernelColor2<<<grid,block>>>(maTextures[2],icpl,maTexSize,step);
