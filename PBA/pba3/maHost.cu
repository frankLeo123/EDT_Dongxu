...
void maInitialization(int textureSize)
{
	...
	maTextures = (short2 **) malloc(5 * sizeof(short2 *)); 
...
{	
	...
	maKernelColorInit1<<<grid,block>>>(maTextures[2],npnt,maTexSize,0,step);
	cudaUnbindTexture(maTexColor);
	...
	while (step>=2)
	{
		...
		maKernelColor1<<<grid,block>>>(maTextures[0],maTextures[2],pnt,maTextures[3],npnt,maTexSize,step2,step);
		...
		cudaBindTexture(0,maTexNpnt,npnt);
		cudaBindTexture(0,maTexColor,maTextures[1]);
		cudaBindTexture(0,maTexLinks,maTextures[3]);
		maKernelColor2<<<grid,block>>>(maTextures[4],npnt,maTexSize,step2,step);
		cudaUnbindTexture(maTexNpnt);
		cudaUnbindTexture(maTexColor);
		cudaUnbindTexture(maTexLinks);
		...
		cudaBindTexture(0,maTexColor,maTextures[4]);
	...
	maKernelTest1<<<grid,block>>>(maTextures[2],npnt,maTexSize);
	cudaBindTexture(0,maTexLinks,maTextures[2]);
	cudaBindTexture(0,maTexIndex,npnt);
